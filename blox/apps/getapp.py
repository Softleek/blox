...
@click.option("--name", type=str, help="Optional name for the cloned app. If not provided, the name will be parsed from the Git URL.")
def getapp(git_url, name):
    ...
    try:
        subprocess.run(["git", "clone", "--depth", "1", git_url, target_dir], check=True)
