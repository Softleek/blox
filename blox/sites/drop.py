...
def dropsite(site):
    ...
    confirm = click.confirm(f"Are you sure you want to delete the site '{selected_site['site_name']}'?", default=False)
    if not confirm:
    ...
    try:
        if os.path.exists(site_folder_path):
            click.echo(f"Attempting to delete the site folder '{site_folder_path}' with admin privileges...")
    ...
    click.echo(f"Site '{selected_site['site_name']}' has been successfully removed from sites.json.")
