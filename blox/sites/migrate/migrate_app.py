...
from .manual_migrations import create_manual_migrations
...
def migrate_app(app_name, django_path):
    ...
    for folder, _ in STRUCTURE.items():
        ...
        if not module_folder_path_abs.startswith(django_path_abs):
            raise ValueError(f"Module folder path '{module_folder_path}' is outside the allowed Django path '{django_path}'.")
    ...
    configure_app(app_name, django_path)
