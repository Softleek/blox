...
def write_model(module_file, fields, model_name, django_path):
    ...
    for field in fields:
        ...
        if field_type == "Select":
            ...
            write_field_declaration(module_file, field_id, "models.BooleanField", field_name=field.get("label", ""))
        elif field_type in  ["Date", "Datetime"]:
            write_field_declaration(module_file, field_id, "models.DateField", field_name=field.get("label", ""))
        elif field_type == "Int":
            write_field_declaration(module_file, field_id, "models.IntegerField", field_name=field.get("label", ""))
        elif field_type == "Float":
            write_field_declaration(module_file, field_id, "models.FloatField", field_name=field.get("label", ""))
        elif field_type in ["Currency", "Percent"]:
            write_field_declaration(module_file, field_id, "models.DecimalField", "max_digits=10, decimal_places=2", field_name=field.get("label", ""))
        elif field_type == "Text":
            write_field_declaration(module_file, field_id, "models.TextField", field_name=field.get("label", ""))
        elif field_type in ["Data", "BarcodeField"]:  # CharFields for Data and BarcodeField
            write_field_declaration(module_file, field_id, "models.CharField", "max_length=255", field_name=field.get("label", ""))
        
        elif field_type == "Duration":
            write_field_declaration(module_file, field_id, "models.DurationField", field_name=field.get("label", ""))
        elif field_type in ["Small Text", "Text Area", "Text", "Long Text", "HTML",  "HTML Editor", "Markdown Editor"]:
            write_field_declaration(module_file, field_id, "models.TextField", field_name=field.get("label", ""))
        elif field_type == "Password":
            write_field_declaration(module_file, field_id, "models.CharField", "max_length=255", field_name=field.get("label", ""))
        elif field_type == "Phone":
            write_field_declaration(module_file, field_id, "models.CharField", "max_length=20", field_name=field.get("label", ""))
        elif field_type == "Rating":
            write_field_declaration(module_file, field_id, "models.DecimalField", "max_digits=2, decimal_places=1", field_name=field.get("label", ""))
        elif field_type == "Signature":
            write_field_declaration(module_file, field_id, "models.CharField", "max_length=255", field_name=field.get("label", ""))
        elif field_type in ["Attach", "Attach Image", "Image", "Barcode"]:
            write_field_declaration(module_file, field_id, "models.FileField", "upload_to='attachments/'", field_name=field.get("label", ""))
        elif field_type == "JSON":
            write_field_declaration(module_file, field_id, "models.JSONField", field_name=field.get("label", ""))
        elif field_type == "Time":
            write_field_declaration(module_file, field_id, "models.TimeField", field_name=field.get("label", ""))
        
        elif field_type not in ["Section Break", "Column Break", "Tab Break"]:  # Skip layout fields
            write_field_declaration(module_file, field_id, "models.CharField", "max_length=255", field_name=field.get("label", ""))
            
        if field_type == "BarcodeField":
                has_barcode_field = True
        

    # Add save method if needed
    if has_barcode_field:
        write_save_method(module_file, fields)
...
def write_field_declaration(module_file, field_id, field_type, extra_params="", field_name=""):
    ...
    if field_type != "models.ManyToManyField":    module_file.write("null=True, blank=True")
    module_file.write(")\n")
...
def write_choices_field(module_file, field, field_type, max_length=None):
    ...
    if choices:
        ...
        for choice in choices:
            sanitized_choice = choice.replace('"', "'")  # Replace internal double quotes with single quotes
        ...
        write_field_declaration(module_file, field_id, field_type, f"choices={options_var}{max_length_param}", field.get("label", ""))
...
def write_link_field(module_file, field, model_name, django_path):
    ...
    write_field_declaration(
        module_file, field_id, "models.ForeignKey", 
        f'"{related_model}", related_name="{related_name}", on_delete=models.CASCADE', field_name=field.get("label", "")
    )
...
def write_table_field(module_file, field, model_name, django_path):
    ...
    write_field_declaration(
        module_file, field_id, "models.ManyToManyField", 
        f'"{related_model}", related_name="{related_name}"', field_name=field.get("label", "")
    )
...
def write_save_method(module_file, fields):
    ...
    for field in fields:
        if field.get("fieldtype") == "BarcodeField":
            field_id = rename_reserved_keywords(field.get("fieldname", ""))
            module_file.write(f"        if not self.{field_id} or not os.path.exists(os.path.join(settings.MEDIA_ROOT, self.{field_id}.name)):\n")
