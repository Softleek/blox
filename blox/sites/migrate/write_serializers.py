...
from ..utils.app_actions import get_doc_details
...
def write_serializers_header(module_file, app_name, module_name, model_name, doc_name, related_fields):
    """
    ...
    module_file.write(f"from {app_name}.models.{module_name}.{doc_name} import {model_name}\n\n")
...
def process_field(field, model_name, current_doc_name):
    ...
    field_id = sanitize_field_name(rename_reserved_keywords(to_snake_case(raw_field_id).lower()))
    ...
    if field_type in ["Link", "ForeignKey", "ManyToManyField", "OneToOneField"]:
        ...
        if not related_model:
            raise ValueError(f"Field {field_id} has a '{field_type}' type but no related model specified.")
...
def write_serializer(module_file, app_name, module_name, model_name, doc_name, doc_folder_path):
    """
    ...
    # Write imports
    write_serializers_header(module_file, app_name, module_name, model_name, doc_name, related_fields)

    # Start defining the main serializer class
    module_file.write(f"class {model_name}Serializer(RelationshipHandlerMixin, serializers.ModelSerializer):\n\n")
